-*-------------------------------------------------------------------------------------------
Computations for Log-conc for morphisms of matroids
...
S = QQ[q][w_0..w_3]
Z = q^(3)*w_1*w_2*w_3 + q^(3)*w_1*w_2*w_0 + q^(3)*w_1*w_3*w_0 + q^(2)*w_2*w_3*w_0 + q^(2)*w_1*w_0^2 + q^(2)*w_2*w_0^2 + q^(1)*w_3*w_0^2 + w_0^3

hessian = method();
hessian(RingElement) := Matrix => f -> (
    S = ring f;
    B = basis(1,S);
...
H = hessian Z
A = sub(H, apply(gens S, r -> r =>1))
sub(A, q=>4)
eigenvalues oo
det A
sub(oo, q=>4)

logConcTest = L -> sub(H, apply(numgens S, i -> S_i => L_i))
logConcTest {1,1,1,1}
eigenvalues sub(oo,q=>2)
#select(oo, i -> i > 0)

apply(1000, i -> (
	L := apply(4, i -> random(1,100));
	E := eigenvalues sub(logConcTest L,q=>5);
	if #select(E, i -> i > 0) > 1 then return L;
	)
    )

logConcTest {20,1,20,1}
eigenvalues sub(logConcTest {20,1,20,1},q=>4)
